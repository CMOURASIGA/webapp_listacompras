...
export function ListsPage({ userId, data, activeList, selectList, reload, startShopping, notify }: { userId: string; data: AppData; activeList?: ShoppingList; selectList: (id: string) => void; reload: () => Promise<void>; startShopping: () => void; notify: (text: string) => void }) {
  ...
  return <div className="space-y-4"><div className="flex items-start justify-between"><div><h2 className="text-2xl font-bold">Minhas listas</h2><p className="text-sm text-slate-500">Planeje diferentes tipos de compra.</p></div><button onClick={() => setShowNew(true)} className="rounded-2xl bg-blue-600 px-4 py-2.5 font-bold text-white">+ Nova</button></div>
    ...
    {activeList && <><Card><div className="flex items-center justify-between"><div><p className="text-xs text-slate-500">Lista ativa</p><h3 className="text-xl font-bold">{activeList.name}</h3></div><button onClick={startShopping} disabled={!items.some(i => i.status === 'pending')} className="rounded-2xl bg-blue-600 px-4 py-2.5 font-bold text-white disabled:opacity-40">Comprar</button></div></Card>
      <Card><h3 className="font-bold">Adição rápida</h3><p className="mb-3 text-sm text-slate-500">Ex.: 2 leite, arroz 5kg, 3 detergentes — depois ajuste quantidade e preço na lista abaixo.</p><div className="flex gap-2"><input value={quick} onChange={e => setQuick(e.target.value)} onKeyDown={e => e.key === 'Enter' && void addQuick()} className="min-w-0 flex-1 rounded-2xl border px-4 py-3" placeholder="Digite os produtos"/><button disabled={busy} onClick={() => void addQuick()} className="rounded-2xl bg-blue-600 px-4 font-bold text-white">Adicionar</button></div><button onClick={() => setNewItem(true)} className="mt-3 w-full rounded-2xl border border-dashed border-slate-300 py-2.5 text-sm font-semibold text-slate-600">+ Adicionar item com quantidade e preço</button></Card>
      <Card><div className="mb-3 flex justify-between"><h3 className="font-bold">Itens</h3><span className="text-sm text-slate-500">{items.filter(i => i.status === 'pending').length} pendentes</span></div><div className="space-y-2">{items.length ? items.map(item => {
        ...
        return <div key={item.id} className={`rounded-2xl border border-slate-100 p-3 ${busyItem === item.id ? 'opacity-60' : ''}`}>
          <div className="flex items-start justify-between gap-2"><strong className={!pending ? 'text-slate-400 line-through' : ''}>{item.name_snapshot}</strong>{pending && <button onClick={() => void remove(item.id)} className="shrink-0 rounded-xl px-3 py-2 text-sm font-semibold text-red-600">Remover</button>}</div>
