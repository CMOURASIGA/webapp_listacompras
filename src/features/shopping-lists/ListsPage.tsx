...
export function ListsPage({ userId, data, activeList, selectList, reload, startShopping, notify }: { userId: string; data: AppData; activeList?: ShoppingList; selectList: (id: string) => void; reload: () => Promise<void>; startShopping: () => void; notify: (text: string) => void }) {
  ...
  // e vira um agrupamento por mês/ano, mostrando só os 3 meses mais recentes.
  const recentHistoryMonths = useMemo(() => {
    const groups = new Map<string, { label: string; lists: ShoppingList[] }>();
    data.lists.filter(isImportedHistoryList).sort((a, b) => +new Date(b.created_at) - +new Date(a.created_at)).forEach(l => {
      const d = new Date(l.created_at); const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
      const label = d.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' }).replace(/^./, c => c.toUpperCase());
      const group = groups.get(key) || { label, lists: [] }; group.lists.push(l); groups.set(key, group);
    });
    return [...groups.entries()].sort((a, b) => b[0].localeCompare(a[0])).slice(0, 3);
  }, [data.lists]);
  ...
  return <div className="space-y-4"><div className="flex items-start justify-between"><div><h2 className="text-2xl font-bold">Minhas listas</h2><p className="text-sm text-slate-500 dark:text-slate-400">Planeje diferentes tipos de compra.</p></div><button onClick={() => setShowNew(true)} className="rounded-2xl bg-brand-600 px-4 py-2.5 font-bold text-white">+ Nova</button></div>
    {ownLists.length ? <div className="flex gap-2 overflow-x-auto pb-1">{ownLists.map(list => { const pendingCount = data.items.filter(i => i.list_id === list.id && i.status === 'pending').length; return <button key={list.id} onClick={() => selectList(list.id)} className={`min-w-[190px] rounded-2xl border p-3 text-left ${activeList?.id === list.id ? 'border-brand-400 bg-brand-50 dark:border-brand-500/50 dark:bg-brand-500/10' : 'bg-white dark:bg-slate-800 dark:border-slate-700'}`}><div className="flex items-center justify-between gap-2"><strong className="truncate">{list.name}</strong><Pill tone={pendingCount === 0 ? 'neutral' : activeList?.id === list.id ? 'brand' : 'neutral'} className="shrink-0">{pendingCount === 0 ? 'Vazia' : `${pendingCount} pendente${pendingCount > 1 ? 's' : ''}`}</Pill></div><p className="mt-1 truncate text-xs text-slate-500 dark:text-slate-400">{list.description || 'Sem descrição'}</p></button>; })}</div> : <EmptyState title="Nenhuma lista criada" description="Toque em “+ Nova”, dê um nome (ex.: Compra do mês) e comece a adicionar itens."/>}
    ...
    {activeList && <><Card><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-xs text-slate-500 dark:text-slate-400">Lista ativa</p><h3 className="truncate text-xl font-bold">{activeList.name}</h3></div><button onClick={startShopping} disabled={!items.some(i => i.status === 'pending')} className="shrink-0 rounded-2xl bg-brand-600 px-4 py-2.5 font-bold text-white disabled:opacity-40">Comprar</button></div></Card>
      ...
      <Card><div className="mb-3 flex justify-between"><h3 className="font-bold">Itens</h3><span className="text-sm text-slate-500 dark:text-slate-400">{items.filter(i => i.status === 'pending').length} pendentes</span></div><div className="space-y-2">{items.length ? items.map(item => {
        ...
        const categoryName = data.categories.find(c => c.id === item.category_id)?.name;
        const Icon = categoryIcon(categoryName);
        return <div key={item.id} className={`rounded-2xl border border-slate-100 p-3 dark:border-slate-700 ${busyItem === item.id ? 'opacity-60' : ''}`}>
          <div className="flex items-start justify-between gap-2"><div className="flex min-w-0 items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-brand-50 text-brand-700 dark:bg-brand-500/15 dark:text-brand-300"><Icon size={16}/></span><strong className={`truncate ${!pending ? 'text-slate-400 line-through' : ''}`}>{item.name_snapshot}</strong></div>{pending && <button onClick={() => void remove(item.id)} className="shrink-0 rounded-xl px-3 py-2 text-sm font-semibold text-danger-600">Remover</button>}</div>
          {pending ? <div className="mt-2 flex flex-wrap items-center gap-3"><div className="flex items-center gap-1 rounded-xl border border-slate-200 dark:border-slate-600"><button onClick={() => void changeQuantity(item, -1)} disabled={Number(item.quantity) <= 1} className="grid h-11 w-11 place-items-center text-lg font-bold text-slate-600 dark:text-slate-300 disabled:opacity-30">−</button><span className="min-w-[2ch] text-center font-semibold">{item.quantity}</span><button onClick={() => void changeQuantity(item, 1)} className="grid h-11 w-11 place-items-center text-lg font-bold text-slate-600 dark:text-slate-300">+</button></div><label className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">R$<input inputMode="decimal" value={priceValue} onChange={e => setPriceDrafts(current => ({ ...current, [item.id]: e.target.value }))} onBlur={() => void commitPrice(item)} placeholder="0,00" className="w-24 rounded-xl border border-slate-200 bg-white px-3 py-2 text-right font-semibold dark:border-slate-600 dark:bg-slate-900"/></label></div>
            : <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{item.quantity}x · {item.estimated_price ? `R$ ${Number(item.estimated_price).toFixed(2)}` : 'sem preço'}</p>}
        </div>;
      }) : <p className="py-6 text-center text-sm text-slate-500 dark:text-slate-400">Adicione o primeiro item acima.</p>}</div></Card></>}
    {showNew && <div onClick={() => setShowNew(false)} className="fixed inset-0 z-50 flex items-end bg-slate-950/40 sm:items-center sm:justify-center"><div onClick={e => e.stopPropagation()} className="w-full rounded-t-3xl bg-white p-5 dark:bg-slate-800 sm:max-w-md sm:rounded-3xl"><h3 className="text-xl font-bold">Nova lista</h3><div className="mt-4 space-y-3"><input autoFocus value={name} onChange={e => setName(e.target.value)} placeholder="Ex.: Compra do mês" className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-900"/><input value={description} onChange={e => setDescription(e.target.value)} placeholder="Descrição opcional" className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-900"/><input type="number" value={budget} onChange={e => setBudget(e.target.value)} placeholder="Orçamento opcional" className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-900"/></div><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={() => setShowNew(false)} className="rounded-2xl border border-slate-200 py-3 font-semibold dark:border-slate-600 dark:text-slate-200">Cancelar</button><button disabled={busy} onClick={() => void createList()} className="rounded-2xl bg-brand-600 py-3 font-bold text-white">Criar</button></div></div></div>}
    {newItem && <div onClick={() => setNewItem(false)} className="fixed inset-0 z-50 flex items-end bg-slate-950/40 sm:items-center sm:justify-center"><div onClick={e => e.stopPropagation()} className="w-full rounded-t-3xl bg-white p-5 dark:bg-slate-800 sm:max-w-md sm:rounded-3xl"><h3 className="text-xl font-bold">Novo item</h3><div className="mt-4 space-y-3"><input autoFocus value={itemName} onChange={e => setItemName(e.target.value)} placeholder="Ex.: Arroz 5kg" className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-600 dark:bg-slate-900"/><div className="flex gap-3"><label className="flex-1 text-sm font-semibold text-slate-600 dark:text-slate-300">Quantidade<input inputMode="decimal" value={itemQty} onChange={e => setItemQty(e.target.value)} className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-center font-normal dark:border-slate-600 dark:bg-slate-900"/></label><label className="flex-1 text-sm font-semibold text-slate-600 dark:text-slate-300">Preço estimado<input inputMode="decimal" value={itemPrice} onChange={e => setItemPrice(e.target.value)} placeholder="Opcional" className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-center font-normal dark:border-slate-600 dark:bg-slate-900"/></label></div></div><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={() => setNewItem(false)} className="rounded-2xl border border-slate-200 py-3 font-semibold dark:border-slate-600 dark:text-slate-200">Cancelar</button><button disabled={busy} onClick={() => void addItem()} className="rounded-2xl bg-brand-600 py-3 font-bold text-white">Adicionar</button></div></div></div>}
  </div>;
}
