...
export function Dashboard({ data, activeList, history, pantry, openList, startShopping, openInsights }: { data: AppData; activeList?: ShoppingList; history: History; pantry: PantryPrediction[]; openList: () => void; startShopping: () => void; openInsights: () => void }) {
  ...
  if (!activeList) return <EmptyState title="Crie sua primeira lista" description="A inteligência de consumo começa quando você planeja e registra uma compra." action={<button onClick={openList} className="rounded-2xl bg-blue-600 px-5 py-3 font-bold text-white">Criar lista</button>}/>;
  return <div className="space-y-4">
    <section className="rounded-3xl bg-gradient-to-br from-blue-800 to-blue-500 p-5 text-white"><p className="text-sm text-blue-100">Próxima compra</p><h2 className="mt-1 text-2xl font-bold">{activeList.name}</h2><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white/10 p-3"><span className="text-xs text-blue-100">{items.length} itens sugeridos</span><strong className="block text-xl">{money(estimate.total)}</strong><span className="text-xs text-blue-100">estimativa</span></div><div className="rounded-2xl bg-white/10 p-3"><span className="text-xs text-blue-100">Cobertura</span><strong className="block text-xl">{estimate.covered}/{estimate.itemCount}</strong><span className="text-xs text-blue-100">com referência</span></div></div><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={openList} className="rounded-2xl bg-white/10 py-3 font-bold">Revisar lista</button><button onClick={startShopping} className="rounded-2xl bg-white py-3 font-bold text-blue-700">Iniciar compra</button></div></section>
    {activeSession && <Card className="border-blue-200"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold uppercase text-blue-600">Compra atual</p><h3 className="font-bold">{sessionPurchases.length} itens registrados</h3><p className="text-sm text-slate-500">{money(sessionPurchases.reduce((s, p) => s + Number(p.total_price), 0))} gastos até agora</p></div><Pill tone="blue">em andamento</Pill></div></Card>}
