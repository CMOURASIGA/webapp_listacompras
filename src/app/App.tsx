...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-slate-100"><header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur dark:border-slate-700 dark:bg-slate-900/90" style={{ paddingTop: 'env(safe-area-inset-top)' }}><div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3"><div className="flex items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-2xl border border-brand-100 bg-white shadow-sm"><img src="/icons/icon-192-v3.png" alt="7Mercado" className="h-full w-full object-cover"/></div><div><div className="flex items-center gap-2"><h1 className="font-bold">7Mercado</h1><span className="hidden text-xs text-slate-400 sm:inline dark:text-slate-500">Consult Services</span></div><p className="text-xs text-slate-500 dark:text-slate-400">{view === 'home' ? 'Visão geral' : view === 'lists' ? 'Planejamento' : view === 'shopping' ? 'Modo mercado' : view === 'prices' ? 'Inteligência de preços' : 'Resumo e despensa'}</p></div></div><div className="flex items-center gap-1"><button onClick={() => setSettings(true)} className="rounded-xl p-2.5 text-slate-600 dark:text-slate-400" aria-label="Configurações"><Settings size={19}/></button><button onClick={() => void supabase.auth.signOut()} className="rounded-xl p-2.5 text-slate-600 dark:text-slate-400" aria-label="Sair"><LogOut size={19}/></button></div></div></header>
