...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900"><header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur" style={{ paddingTop: 'env(safe-area-inset-top)' }}><div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3"><div className="flex items-center gap-3"><div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-2xl border border-brand-100 bg-white shadow-sm"><img src="/icons/icon-192-v2.png" alt="7Mercado" className="h-full w-full object-cover"/></div><div><div className="flex items-center gap-2"><h1 className="font-bold">7Mercado</h1><span className="hidden text-xs text-slate-400 sm:inline">Consult Services</span></div><p className="text-xs text-slate-500">{view === 'home' ? 'Visão geral' : view === 'lists' ? 'Planejamento' : view === 'shopping' ? 'Modo mercado' : view === 'prices' ? 'Inteligência de preços' : 'Resumo e despensa'}</p></div></div><div className="flex items-center gap-1"><button onClick={() => setSettings(true)} className="rounded-xl p-2.5 text-slate-600" aria-label="Configurações"><Settings size={19}/></button><button onClick={() => void supabase.auth.signOut()} className="rounded-xl p-2.5 text-slate-600" aria-label="Sair"><LogOut size={19}/></button></div></div></header>
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-30 border-t border-slate-200 bg-white/95 backdrop-blur" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}><div className="mx-auto grid max-w-3xl grid-cols-5">{nav.map(([key, label, Icon]) => <button key={key} onClick={() => setView(key)} className={`flex min-h-[3rem] flex-col items-center justify-center gap-1 py-3 text-xs font-semibold ${view === key ? 'text-brand-700' : 'text-slate-500'}`}><Icon size={21}/>{label}</button>)}</div></nav>
